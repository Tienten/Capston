...
def parse_args():
    ...
    parser.add_argument('--fer2013', type=str, default='../data_2/FER2013/', help='dataset path.')
...
class FER2013Dataset(data.Dataset):
    def __init__(self, root_dir, phase, transform=None, basic_aug=False):
        ...
        self.targets = []
        self.labels = []
        self.label_map = {'surprise': 0, 'fear': 1, 'disgust': 2, 'happy': 3, 
            ...
            if os.path.isdir(label_dir) and (self.phase in label_dir):
                for f in os.listdir(label_dir):
                    if f.endswith('.jpg'):
                        ...
                        if self.phase == "train":
                            if(f[0:5]=='angry'):
                                self.labels.append(int(self.label_map['angry']))
                            elif(f[0:4]=='fear'):
                                self.labels.append(int(self.label_map['fear']))
                            elif(f[0:7]=='disgust'):
                                self.labels.append(int(self.label_map['disgust']))
                            elif(f[0:5]=='happy'):
                                self.labels.append(int(self.label_map['happy']))
                            elif(f[0:7]=='neutral'):
                                self.labels.append(int(self.label_map['neutral']))
                            elif(f[0:3]=='sad'):
                                self.labels.append(int(self.label_map['sad']))
                            elif(f[0:8]=='surprise'):
                                self.labels.append(int(self.label_map['surprise']))
...
def run_training():
    ...
    train_dataset = FER2013Dataset(args.fer2013, phase = 'train', transform = data_transforms['train'], basic_aug = True)
