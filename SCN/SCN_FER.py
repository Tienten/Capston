...
def parse_args():
    ...
    parser.add_argument('--fer2013', type=str, default='../data_1/FER2013/', help='dataset path.')
...
class FER2013Dataset(data.Dataset):
    def __init__(self, root_dir, phase, transform=None, basic_aug=False):
        ...
        # Assume directories are named after the labels
        # for phase in os.listdir(root_dir):
            ...
            if os.path.isdir(label_dir) and (self.phase in label_dir):
                for f in os.listdir(label_dir):
                    if f.endswith('.jpg'):
                        ...
                        self.labels.append(int(self.label_map[label]))
...
def run_training():
    ...
    train_loader = torch.utils.data.DataLoader(train_dataset,
                                               ...
                                               shuffle = True,  
                                               pin_memory = True)
    test_dataset = FER2013Dataset(args.fer2013, phase = 'test', transform = data_transforms['test'])   
    ...
    for i in range(1, args.epochs + 1):
        ...
        with torch.no_grad():
            ...
            testing_acc.append(float('%.4f'%(acc)))
